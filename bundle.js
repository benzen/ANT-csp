...
var FOOD, _, ant, ants, config, csp, drawMap, status, statusChan;
...
csp.go(function*() {
  var results, statusLine;
  results = [];
  while (true) {
    statusLine = (yield statusChan);
    results.push(drawMap(statusLine));
  }
  return results;
});
...
},{"./lib/ant":2,"./lib/config":3,"./lib/food":4,"./lib/render":6,"./lib/status":8,"js-csp":13,"underscore":22}],2:[function(require,module,exports){
var TIMER, _, ant, config, csp, food, marks, river, utils;
...
ant = function() {
  "Ant process";
  var bag, chan, distance, followTrack, generatePosition, goHome, isHomeWithFood, isNotPreviousPosition, isValidPosition, keepSearching, move_to, nextPos, nextPosToHome, onFood, onTick, onTrack, position, previous_position, takeHome;
  position = {
    x: config.anthill.x,
    y: config.anthill.y
  };
  ...
  isValidPosition = function(p) {
    ...
    return utils.isInWorldBounds(p) && river.isInRiver(p);
  };
  isNotPreviousPosition = function(p) {
    return !utils.isSamePos(p, previous_position);
  };
  ...
  nextPos = function() {
    ...
    return _.chain(generatePosition()).filter(isValidPosition).filter(isNotPreviousPosition).shuffle().sortBy(function(p) {
      return food.isThereFood(p);
    }).first().value();
  };
  move_to = function(p) {
    ...
    return ref = [position, p], previous_position = ref[0], position = ref[1], ref;
  };
  onTrack = function() {
    "is there a mark on my current Position";
    return !_.isEmpty(marks.snort(position));
  };
  followTrack = function() {
    "go to the other end of the mark. If it's the end of the track find a random position.";
    var followedMark;
    followedMark = _.chain(marks.snort(position)).filter(function(m) {
      return isNotPreviousPosition(m.to);
    }).filter(function(m) {
      return m.level % 2;
    }).first().value();
    if (followedMark) {
      return move_to(followedMark.to);
    } else {
      return keepSearching();
    }
  };
  nextPosToHome = function() {
    "find a position that go closer to the anthill";
    return _.chain(generatePosition()).filter(isValidPosition).filter(isNotPreviousPosition).sortBy(function(p) {
      return distance(config.anthill, p);
    }).first(3).shuffle().first().value();
  };
  goHome = function() {
    "move to a position closer to anthill and put a mark";
    marks.mark(position, previous_position);
    return move_to(nextPosToHome());
  };
  takeHome = function() {
  ...
  onFood = function() {
    ...
    return !!food.isThereFood(position);
  };
  isHomeWithFood = function() {
    "What's on tv?";
    return bag.length && utils.isSamePos(position, config.anthill);
  };
  keepSearching = function() {
    "Choose a new random position";
    return move_to(nextPos());
  };
  onTick = function() {
...
},{"./config":3,"./food":4,"./marks":5,"./river":7,"./timer":9,"./utils":10,"js-csp":13,"underscore":22}],3:[function(require,module,exports){
module.exports = {
  anthill: {
    x: 40,
    y: 40
  ...
  MARKS_LEVEL_INCREMENT: 100,
  MARKS_LEVEL_DECREMENT: 1
};



},{}],4:[function(require,module,exports){
var FOOD, FOODS, _, config, food, generateFood, isThereFood, utils;
...
generateFood = function(nbFood) {
  return _.times(nbFood, function() {
    var x, y;
...
isThereFood = function(pos) {
  "find a food (with amount superior to 0) at the given position";
  return _.find(FOODS, function(f) {
    return f.amount > 0 && utils.isSamePos(f.pos, pos);
  });
};
...
},{"./config":3,"./utils":10,"underscore":22}],5:[function(require,module,exports){
var MARKS, TIMER, _, config, csp, evaporation, isThereMark, mark, snort, utils;
...
snort = function(p) {
  ...
  return _.filter(MARKS, function(m) {
    return m.level > 0 && utils.isSamePos(m.from, p);
  });
};
...
mark = function(from, to) {
  ...
  var previousMark;
  previousMark = _.find(MARKS, function(m) {
    return utils.isSamePos(m.from, from) && utils.isSamePos(m.to, to);
  });
  if (previousMark) {
    return previousMark.level += config.MARKS_LEVEL_INCREMENT;
  } else {
    return MARKS.push({
      from: from,
      to: to,
...
isThereMark = function(pos) {
  return _.find(MARKS, function(m) {
    return m.level > 0 && utils.isSamePos(m.from, pos);
  });
};
...
evaporation = function() {
  ...
  return csp.go(function*() {
    ...
    while (true) {
      (yield csp.take(localTimer));
      results.push(_.each(MARKS, function(m) {
        return m.level -= 1;
      }));
    }
    return results;
...
},{"./config":3,"./timer":9,"./utils":10,"js-csp":13,"underscore":22}],6:[function(require,module,exports){
var $, _, black, blue, brown, cellSize, config, deg_to_rad, direction, direction_to_rad, drawMap, food, getContext, green, height, marks, red, river, scale, width;
...
getContext = function(mapWidth, mapHeight) {
...
drawMap = function(antsPositions) {
  "Draw everything; background, marks, anthill, ants, foods";
  var ctx;
  ctx = getContext();
  ctx.fillStyle = 'green';
  ctx.fillRect(0, 0, scale(config.world_dim.x), scale(config.world_dim.y));
  _.each(marks.MARKS(), function(m) {
    ctx.fillStyle = "rgba(170,255, 234, " + (m.level / 1000) + ")";
    return ctx.fillRect(scale(m.from.x), scale(m.from.y), cellSize, cellSize);
  });
  ctx.fillStyle = brown;
  ctx.fillRect(scale(config.anthill.x), scale(config.anthill.y), cellSize, cellSize);
  _.each(antsPositions, function(arg) {
    var angle, dir, img, position, previous_position, trX, trY;
    position = arg.position, previous_position = arg.previous_position;
  ...
  _.each(food.FOODS, function(f) {
    if (f.amount > 0) {
      ctx.fillStyle = red;
      return ctx.fillRect(scale(f.pos.x), scale(f.pos.y), cellSize, cellSize);
    }
  });
  return _.each(river.RIVER, function(r) {
    ctx.fillStyle = "blue";
    return ctx.fillRect(scale(r.x), scale(r.y), cellSize, cellSize);
  });
};
...
},{"./config":3,"./food":4,"./marks":5,"./river":7,"jquery":11,"underscore":22}],7:[function(require,module,exports){
var RIVER, _, config, generateRiver, isInRiver, memoIsInRiver, utils;
...
generateRiver = function(start, length) {
  var generateRiverRec, river;
  river = [];
  generateRiverRec = function(start, acc) {
    var np;
    if (acc.length === length) {
      return acc;
    }
    np = _.chain(utils.contigous(start)).filter(utils.isInWorldBounds).shuffle().first().value();
    acc.push(np);
    return generateRiverRec(np, acc);
  };
...
memoIsInRiver = _.memoize(function(p) {
  return !_.any(RIVER, function(cp) {
    return utils.isSamePos(p, cp);
  });
}, function(p) {
  return JSON.stringify(p);
});

isInRiver = function(p) {
  return memoIsInRiver(p);
};
...
},{"./config":3,"./utils":10,"underscore":22}],8:[function(require,module,exports){
var csp, status;

csp = require('js-csp');

status = function(ants) {
  "Take each ants's positions and put it into a single channel";
  var chan;
  chan = csp.chan();
  csp.go(function*() {
    var ant, i, index, len, positions, results;
    results = [];
    while (true) {
      positions = [];
      for (index = i = 0, len = ants.length; i < len; index = ++i) {
        ant = ants[index];
        positions[index] = (yield csp.take(ant));
      }
      results.push((yield csp.put(chan, positions)));
    }
    return results;
  });
  return chan;
...
},{"js-csp":13}],9:[function(require,module,exports){
var config, csp, timer;
...
timer = function() {
  ...
  csp.go(function*() {
    ...
    while (true) {
      ...
      results.push((yield csp.put(chan, 'TICK')));
    }
    return results;
  });
  return chan;
...
},{"./config":3,"js-csp":13}],10:[function(require,module,exports){
var config, contigous, isInWorldBounds, isSamePos;

config = require('./config');

contigous = function(position) {
  return [
    {
      x: position.x + 1,
      y: position.y
...
isInWorldBounds = function(p) {
  return p.x >= 0 && p.y >= 0 && p.x <= config.world_dim.x && p.y <= config.world_dim.y;
};
...
},{"./impl/buffers":16,"./impl/channels":17,"./impl/process":19,"./impl/select":20,"./impl/timers":21}],13:[function(require,module,exports){
"use strict";
...
},{"./csp.core":12,"./csp.operations":14,"./csp.pipeline":15}],14:[function(require,module,exports){
"use strict";
...
},{"./csp.core":12,"./impl/channels":17}],15:[function(require,module,exports){
"use strict";
...
},{"./csp.core":12}],16:[function(require,module,exports){
"use strict";
...
},{"./buffers":16,"./dispatch":18}],18:[function(require,module,exports){
"use strict";
...
},{"./buffers":16}],19:[function(require,module,exports){
"use strict";
...
},{"./channels":17,"./dispatch":18,"./select":20}],20:[function(require,module,exports){
"use strict";
...
},{"./channels":17}],21:[function(require,module,exports){
"use strict";
...
},{"./channels":17,"./dispatch":18}],22:[function(require,module,exports){
//     Underscore.js 1.8.3
